    ...
    public class AlunoService : IAlunoService
    {
        ...
        public async Task<ServiceResponse<GetAlunoDTO>> GetAlunoById(int id)
        {
            ...
            try
            {
                ...
                var aluno = await _context.Alunos.Include(a => a.Materias).ThenInclude(m => m.Professor).FirstOrDefaultAsync(item => item.Id == id)
        ...
        public async Task<ServiceResponse<GetAlunoDTO>> UpdateAluno(UpdateAlunoDTO updatedAluno, int id)
        {
            ...
            try
            {
                ...
                var dbAlunos = await _context.Alunos.FirstOrDefaultAsync(item => item.Id == id)
                    ?? throw new KeyNotFoundException("Aluno não encontrado");
        ...
        public async Task<ServiceResponse<bool>> DeleteAluno(int id)
        {
            ...
            {
                // Find the Aluno in the database by ID
                var dbAluno = await _context.Alunos.FirstOrDefaultAsync(item => item.Id == id)
                    ?? throw new KeyNotFoundException("Aluno não encontrado");
        ...
        public async Task LinkAlunoToMateria(LinkMateriaAlunoDTO newLinkMateriaAluno)
        {
            ...
            var aluno = await _context.Alunos
                ...
                .FirstOrDefaultAsync(a => a.Id == newLinkMateriaAluno.AlunoId) ?? throw new KeyNotFoundException("Aluno não encontrado.");
